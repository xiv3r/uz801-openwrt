#!/bin/sh -e
# mk_gpt_both0.sh
# - Usa TOT_SECTORS=7569408 (sectores lógicos de 512B) para fijar last-lba y el header GPT de respaldo.
# - Para obtener TOT_SECTORS automáticamente desde EDL:
...
# Tamaño total en sectores de 512B (aportado)
TOT_SECTORS=7569408

# GPT: first-lba y last-lba
FIRST_LBA=34
LAST_LBA=$((TOT_SECTORS - 34))

# rootfs 128 MiB -> 128*1024*1024/512 = 262144 sectores
ROOTFS_START=348194
ROOTFS_SIZE=262144
ROOTFS_END=$((ROOTFS_START + ROOTFS_SIZE - 1))

# rootfs_data ocupa el resto hasta last-lba
ROOTFSDATA_START=$((ROOTFS_END + 1))
ROOTFSDATA_SIZE=$((LAST_LBA - ROOTFSDATA_START + 1))

# Comprobaciones básicas
[ ${ROOTFSDATA_SIZE} -gt 0 ] || { echo "ERROR: No hay espacio para rootfs_data"; exit 1; }

# Crear imagen con el tamaño exacto
truncate -s $((TOT_SECTORS * 512)) "${IMG}"

# Tabla GPT vía heredoc directa a sfdisk
sfdisk "${IMG}" <<EOF
label: gpt
...
# rootfs_data consume el resto útil hasta last-lba
gpt.img15 : start=${ROOTFSDATA_START}, size=${ROOTFSDATA_SIZE}, type=1B81E7E6-F50D-419B-A739-2AEEF8DA3335, name="rootfs_data"
EOF

# Construir gpt_both0.bin (primario + entradas + header GPT de respaldo)
